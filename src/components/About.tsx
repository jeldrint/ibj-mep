      ...
      <div className="w-full h-full">
         ...
         <section id='about-footer' ref={ref4} className={`${isInViewport4 ? 'animate-in duration-1.5s fade-in-5 slide-in-from-bottom-20' : 'invisible'} relative h-[40vh] bg-cover bg-bottom lg:bg-[0%_85%] w-full bg-blue-800 bg-blend-hard-light`} style={{backgroundImage: `url(${pic2})`}}>
            <div className='opacity-85 absolute p-9 h-full w-full flex flex-col gap-y-2 items-center justify-center backdrop-blur-sm'>
               ...
               <span className='text-sm md:text-base lg:text-lg text-black text-center max-w-[900px] bg-slate-100 p-[1px] rounded-md'>By subscribing to the IBJ newsletter, you'll receive regular updates, news, and job openings delivered straight to your inbox.</span>
               <form className='relative'>
                  <input className='min-w-[256px] md:min-w-[350px] p-3 rounded-md' name='email' id='email' placeholder='Enter your E-mail Address' type='email' required />
                  <button type='submit' className='absolute p-3 rounded-md bg-indigo-700 active:bg-indigo-900'>SUBMIT</button>
               </form>
            </div>
         </section>   
