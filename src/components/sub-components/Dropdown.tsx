...
const DropDown = ({navName, toggleDropdown, setToggleDropdown, newNavName, dropdownItems}: DropdownProps) => {
   ...
   // ** Dropdown for the smaller screens
      ...
      return(
         <div className={`${!toggleDropdown[newNavName as keyof typeof toggleDropdown] ? 'hidden' : 'flex animate-in fade-in-5 slide-in-from-top-7' } flex-col duration-1000 px-2 rounded cursor-pointer text-right sm:text-left text-xs sm:text-base bg-transparent text-slate-50`}>
            {
               dropdownItems.map((item, idx)=>{
                  const path: string = item.replace(/\s+/g, '-').toLowerCase();
